...
class Plate(object):
    ...
    def __init__(self, config_file):
        ...
        self._gen_loading(conf_pl, conf_ld, conf_mo)
    ...
    def compute_stresses(self):
        ...
        self.m_x = np.zeros([self.num_x_elem + 1, self.num_y_elem])
        self.m_y = np.zeros([self.num_x_elem + 1, self.num_y_elem])
        self.m_xy = np.zeros([self.num_x_elem + 1, self.num_y_elem])

        self.deflection = np.vstack([np.zeros(self.num_y_elem), self.deflection])

        # stencils used here
        p = self.p
        p2 = self.p**2
        self.mx_sten = np.array([[0, 1, 0], [p, -2-2*p, p], [0, 1, 0]])
        self.my_sten = np.array([[0, p, 0], [1, -2-2*p, 1], [0, p, 0]])
        self.mxy_sten = np.array([[1, 0, -1], [0, 0, 0], [-1, 0, 1]])

        # mxy stencils
        self.mxy_r1_l = np.array([[-p, 0], [-2-2*p, 2], [p, 0]])
        self.mxy_r1_r = -np.fliplr(self.mxy_r1_l)
        self.mxy_r2up_l = np.array([[-p, 0], [2+2*p, -2], [0, 0], [-2-2*p, 2], [p, 0]])
        self.mxy_r2up_r = -np.fliplr(self.mxy_r2up_l)
        self.mxy_rpen_l = np.array([[-p, 0], [2+2*p, -2], [-p, 0], [-2, 2]])
        self.mxy_rpen_r = -np.fliplr(self.mxy_rpen_l)
        self.mxy_rl_l = np.array([[2, 0, -2, 0], [-2, -p, 2+2*p, -p]])
        self.mxy_rl_r = -np.fliplr(self.mxy_rl_l)
        self.mxy_rl = np.array([[0, 2, 0, -2, 0], [p, -2-2*p, 0, 2+2*p, -p]])

        # separate computation of m_x and m_y from m_xy - m_xy is such a pain in the ass
        for x in range(1, self.num_x_elem + 1):
            for y in range(self.num_y_elem):
                if x == 1: # note that x == 0 is just a dummy
                    if y in [0, self.num_y_elem -1]:
                        self.m_y[x, y] = 0
                        self.m_x[x, y] = -self.rigidity*(1-p2)*(-2*self.deflection[x, y] + self.deflection[x+1, y])/self.delta**2
                    else:
                        self.m_x[x, y] = -self.rigidity*np.sum(self.deflection[x-1:x+2, y-1:y+2]*self.mx_sten)/self.delta**2
                        self.m_y[x, y] = -self.rigidity*np.sum(self.deflection[x-1:x+2, y-1:y+2]*self.my_sten)/self.delta**2
                elif x == self.num_x_elem:
                    if y in [0, self.num_y_elem -1]:
                        self.m_x[x, y] = self.edge_moment[y]
                        self.m_y[x, y] = 0
                    else:
                        self.m_x[x, y] = self.edge_moment[y]
                        self.m_y[x, y] = -self.rigidity*(1-p2)*(self.deflection[x, y-1] - 2*self.deflection[x, y] + self.deflection[x, y+1])/self.delta**2
                        self.m_y[x, y] += p*self.edge_moment[y]
                else:
                    if y in [0, self.num_y_elem -1]:
                        self.m_y[x, y] = 0
                        self.m_x[x, y] = -self.rigidity*(1-p2)*(self.deflection[x-1,y] - 2*self.deflection[x, y] + self.deflection[x+1, y])/self.delta**2
                    else:
                        self.m_x[x, y] = -self.rigidity*np.sum(self.deflection[x-1:x+2, y-1:y+2]*self.mx_sten)/self.delta**2
                        self.m_y[x, y] = -self.rigidity*np.sum(self.deflection[x-1:x+2, y-1:y+2]*self.my_sten)/self.delta**2
        ...
        for x in range(1, self.num_x_elem + 1):
            for y in range(self.num_y_elem):
                if x == 1:
                    if y == 0:
                        self.m_xy[x, y] = (self.rigidity*(1-p)/(4*p**2))*np.sum(self.deflection[x:x+3, y:y+2]*self.mxy_r1_l)
                    elif y == self.num_y_elem - 1:
                        self.m_xy[x, y] = (self.rigidity*(1-p)/(4*p**2))*np.sum(self.deflection[x:x+3, y-1:y+1]*self.mxy_r1_r)
                    else:
                        self.m_xy[x, y] = (self.rigidity*(1-p)/(4*p**2))*np.sum(self.deflection[x-1:x+2, y-1:y+2]*self.mxy_sten)
                elif x == self.num_x_elem - 1:
                    if y == 0:
                        self.m_xy[x, y] = (self.rigidity*(1-p)/(4*p**2))*np.sum(self.deflection[x-2:x+2, y:y+2]*self.mxy_rpen_l) - p*self.m_x[x+1, y]/(4*(1+p))
                    elif y == self.num_y_elem - 1:
                        self.m_xy[x, y] = (self.rigidity*(1-p)/(4*p**2))*np.sum(self.deflection[x-2:x+2, y-1:y+1]*self.mxy_rpen_r) + p*self.m_x[x+1, y]/(4*(1+p))
                    else:
                        self.m_xy[x, y] = (self.rigidity*(1-p)/(4*p**2))*np.sum(self.deflection[x-1:x+2, y-1:y+2]*self.mxy_sten)
                elif x == self.num_x_elem:
                    if y in [0, self.num_y_elem - 1]:
                        ...
                        self.m_xy[x, y] = (self.rigidity*(1-p)/(4*p**2))*np.sum(self.deflection[x-1:x+1, y-1:y+3]*self.mxy_rl_l)
                        self.m_xy[x, y] += self.m_x[x, y-1]/(4*(1+p)) - (1-p)*self.m_x[x, y+1]/4 - p*self.m_y[x, y-1]/(4*(1+p))
                    elif y == self.num_y_elem - 2:
                        self.m_xy[x, y] = (self.rigidity*(1-p)/(4*p**2))*np.sum(self.deflection[x-1:x+1, y-2:y+2]*self.mxy_rl_r)
                        self.m_xy[x, y] += -self.m_x[x, y+1]/(4*(1+p)) + (1-p)*self.m_x[x, y-1]/4 + p*self.m_y[x, y+1]/(4*(1+p))
                    else:
                        self.m_xy[x, y] = (self.rigidity*(1-p)/(4*p**2))*np.sum(self.deflection[x-1:x+1, y-2:y+3]*self.mxy_rl)
                        self.m_xy[x, y] += (1-p)*(self.m_x[x, y-1] - self.m_x[x, y+1])/4
                else:
                    if y == 0:
                        self.m_xy[x, y] = (self.rigidity*(1-p)/(4*p**2))*np.sum(self.deflection[x-2:x+3, y:y+2]*self.mxy_r2up_l)
                    elif y == self.num_y_elem - 1:
                        self.m_xy[x, y] = (self.rigidity*(1-p)/(4*p**2))*np.sum(self.deflection[x-2:x+3, y-1:y+1]*self.mxy_r2up_r)
                    else:
                        #print(x, y)
                        #print('deflection shape', self.deflection[x-1:x+2, y-1:y+2].shape)
                        #print('mxy shape',self.mxy_sten.shape)
                        self.m_xy[x, y] = (self.rigidity*(1-p)/(4*p**2))*np.sum(self.deflection[x-1:x+2, y-1:y+2]*self.mxy_sten)

        self.sigma_x = -self.stress_const*self.m_x[1:, :]
        self.sigma_y = -self.stress_const*self.m_y[1:, :]
        self.sigma_xy = self.stress_const*self.m_xy[1:, :]
        self.m_x = self.m_x[1:, :]
        self.m_y = self.m_y[1:, :]
        self.m_xy = self.m_xy[1:, :]
        self.deflection = self.deflection[1:, :]
