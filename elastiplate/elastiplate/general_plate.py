...
class GeneralPlate(object):
    ...
    def __init__(self, config_file, pl_type = 'elastic'):
        ...
        if pl_type == 'elastic':
            ...
            self.alpha = 2*np.pi*conf_pl["thickness"]*(conf_pl["plate_viscosity"]/(3*conf_pl["mantle_viscosity"]))**(1/3)
            self.iso_term = (8*np.pi*conf_pl["mantle_viscosity"])/(self.rigidity*self.alpha)
            self.stress_const = 1
        else:
            print('Error: unsupported plate type...')
            return
    ...
    def write_out(self, out_fname):
        ...
        out_file.attrs["plate_type"] = self.pl_type
